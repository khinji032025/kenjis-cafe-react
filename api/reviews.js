...
export default async function handler(req, res) {
  ...
  try {
    if (endpoint === 'check') {
      ...
      const rows = await sql`SELECT * FROM reviews WHERE order_id = ${order_id}`;
    ...
    if (endpoint === 'submit') {
      ...
      const orders = await sql`SELECT * FROM orders WHERE order_id = ${order_id.toUpperCase()} AND status = 'Completed'`;
      if (!orders.length) return res.status(400).json({ success: false, message: 'Only completed orders can be reviewed.' });
      const existing = await sql`SELECT id FROM reviews WHERE order_id = ${order_id.toUpperCase()}`;
      if (existing.length > 0) return res.status(400).json({ success: false, message: 'You have already reviewed this order.' });
      await sql`INSERT INTO reviews (order_id, customer_name, rating, comment) VALUES (${order_id.toUpperCase()}, ${orders[0].customer_name}, ${rating}, ${comment || ''})`;
